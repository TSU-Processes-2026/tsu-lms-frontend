...
const SubjectView = () => {
    ...
    const { config, isConfigLoading, errorMessage: configError } = useLoadConfig(subjectId ?? '', userRole);
                        ...
                        <div className='flex flex-col gap-2 my-2'>
                            ...
                            {isConfigLoading && (
                                <div className='p-4 bg-blue-50 border-b-blue-50 rounded-xl border border-blue-100 backdrop-blur-sm shadow-md'>
                                    <p className='text-sm text-blue-700'>Загрузка настроек команд...</p>
                                </div>
                            )}
                            ...
                            <ul className='grid sm:grid-cols-1 md:grid-col </ul>s-2 lg:grid-cols-2 gap-4 mt-4'>
                                {teams.map((item, index) => (
                                    <CommandCard
                                        ...
                                        captainName={
                                            item.members.find((participant) => participant.userId === item.captainId)
                                                ?.username ?? null
                                        }
                                        decisionInfo={
                                            item.finalDecision
