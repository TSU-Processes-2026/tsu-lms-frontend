...
interface Props {
  ...
  onUpsertInstructor: (criterionId: string, value: number | boolean) => void;
  isTeacher: boolean;
}
    ...
    {criteria.map((criterion) => {
      ...
      return <div key={criterion.id} className='border rounded-xl p-3'>
        ...
        {isTeacher && <div className='mt-2 flex gap-2'>
          <button onClick={() => onUpsertInstructor(criterion.id, true)} className='px-3 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-xs'>✔</button>
          <button onClick={() => onUpsertInstructor(criterion.id, false)} className='px-3 py-1 rounded-lg bg-red-100 text-red-700 text-xs'>✖</button>
