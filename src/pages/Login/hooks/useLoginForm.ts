...
export const useLoginForm = () => {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            localStorage.setItem('userId', response.data.userId.trim());
            localStorage.setItem('sessionId', response.data.sessionId.trim());
            navigate(HOME_PAGE_URL);
        } catch (error) {
            if (isAxiosError(error)) {
                setErrorMessage(error.response?.data.message || 'Неверный логин или пароль');
            } else {
