...
export function useSubjectView(): UseSubjectViewResult {
  ...
  const addComment = async (postId: string): Promise<void> => {
    ...
    try {
      ...
      setErrorByPostId((prev: Record<string, string | null>) => ({ ...prev, [postId]: 'Ошибка добавления комментария' }));
    }
  };
   ...
   * @param type
   */
    const mapQuestionType = (type: 'single' | 'multiple' | 'input'): import('@/types/subject/AssignmentCreate').AssignmentQuestionType => {
